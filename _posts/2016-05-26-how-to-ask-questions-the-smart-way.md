...
- [3\. 在提问之前](#3-在提问之前)
- [4\. 当你提问时](#4-当你提问时)
    - [4.1. 慎选提问的论坛](#41-慎选提问的论坛)
    - [4.2. Stack Overflow](#42-stack-overflow)
    - [4.3. 网站和IRC论坛](#43-网站和irc论坛)
    - [4.4. 第二步，使用项目邮件列表](#44-第二步使用项目邮件列表)
    - [4.5. 使用有意义且描述明确的标题](#45-使用有意义且描述明确的标题)
    - [4.6. 使问题容易回复](#46-使问题容易回复)
    - [4.7. 用清晰、正确、精准并语法正确的语句](#47-用清晰正确精准并语法正确的语句)
    - [4.8. 使用易于读取且标准的文件格式发送问题](#48-使用易于读取且标准的文件格式发送问题)
    - [4.9. 精确的描述问题并言之有物](#49-精确的描述问题并言之有物)
    - [4.10. 话不在多而在精](#410-话不在多而在精)
    - [4.11. 别动辄声称找到Bug](#411-别动辄声称找到bug)
    - [4.12. 可以低声下气，但还是要先做功课](#412-可以低声下气但还是要先做功课)
    - [4.13. 描述问题症状而非猜测](#413-描述问题症状而非猜测)
    - [4.14. 按发生时间先后列出问题症状](#414-按发生时间先后列出问题症状)
    - [4.15. 描述目标而不是过程](#415-描述目标而不是过程)
    - [4.16. 别要求使用私人电邮回复](#416-别要求使用私人电邮回复)
    - [4.17. 清楚明确的表达你的问题以及需求](#417-清楚明确的表达你的问题以及需求)
    - [4.18. 询问有关代码的问题时](#418-询问有关代码的问题时)
    - [4.19. 别把自己家庭作业的问题贴上来](#419-别把自己家庭作业的问题贴上来)
    - [4.20. 去掉无意义的提问句](#420-去掉无意义的提问句)
    - [4.21. 即使你很急也不要在标题写`紧急`](#421-即使你很急也不要在标题写紧急)
    - [4.22. 礼多人不怪，而且有时还很有帮助](#422-礼多人不怪而且有时还很有帮助)
    - [4.23. 问题解决后，加个简短的补充说明](#423-问题解决后加个简短的补充说明)
- [5\. 如何解读答案](#5-如何解读答案)
    - [5.1. RTFM和STFW：如何知道你已完全搞砸了](#51-rtfm和stfw如何知道你已完全搞砸了)
    - [5.2. 如果还是搞不懂](#52-如果还是搞不懂)
    - [5.3. 处理无礼的回应](#53-处理无礼的回应)
- [6\. 如何避免扮演失败者](#6-如何避免扮演失败者)
- [7\. 不该问的问题](#7-不该问的问题)
- [8\. 好问题与蠢问题](#8-好问题与蠢问题)
- [9\. 如果得不到回答](#9-如果得不到回答)
- [10\. 如何更好地回答问题](#10-如何更好地回答问题)
- [11\. 相关资源](#11-相关资源)
- [12\. 鸣谢](#12-鸣谢)
...
> （欢迎对本指南提出改进意见。你可以 email 你的建议至 [esr@thyrsus.com](mailto:esr@thyrsus.com) 或 [respond-auto@linuxmafia.com](mailto:respond-auto@linuxmafia.com)。然而请注意，本文并非[网络礼节](https://www.ietf.org/rfc/rfc1855.txt)的通用指南，而我们通常会拒绝无助于在技术论坛得到有用答案的建议。）
...
这样做的用处至少有三点。 第一，表现出你为简化问题付出了努力，这可以使你得到回答的机会增加； 第二，简化问题使你更有可能得到**_有用_**的答案； 第三，在精炼你的bug报告的过程中，你很可能就自己找到了解决方法或权宜之计。
...
请记得，还有许多其它使用者没遇到你发现的问题，否则你在阅读文件或搜索网页时就应该发现了（你在抱怨前[已经做了这些，是吧](#12-鸣谢)？）。这也意味着很有可能是你弄错了而不是软件本身有问题。
...
> 我的组装电脑是 FIC-PA2007 主机板搭载 AMD K6/233 CPU（威盛 Apollo VP2芯片组）， 256MB Corsair PC133 SDRAM内存，在编译内核时，从开机20分钟以后就频频产生 SIG11 错误， 但是在头20分钟内从没发生过相同的问题。重新启动也没有用，但是关机一晚上就又能工作20分钟。 所有内存都换过了，没有效果。相关部分的标准编译记录如下…。
...
最有效描述程序问题的方法是提供最精简的Bug展示测试示例（bug-demonstrating test case）。什么是最精简的测试示例? 那是问题的缩影；一小个程序片段能**刚好**展示出程序的异常行为，而不包含其他令人分散注意力的内容。怎么制作最精简的测试示例？如果你知道哪一行或哪一段代码会造成异常的行为，复制下来并加入足够重现这个状况的代码（例如，足以让这段代码能被编译/直译/被应用程序处理）。如果你无法将问题缩减到一个特定区块，就复制一份代码并移除不影响产生问题行为的部分。总之，测试示例越小越好（查看[话不在多而在精](#12-鸣谢)一节）。
...
回答：如果你有足够的智慧提这个问题，你也该有足够的智慧去 [RTFM](#51-rtfm和stfw如何知道你已完全搞砸了)，然后自己去找出来。
...
这种问法无非想得到 [STFW](#51-rtfm和stfw如何知道你已完全搞砸了) 这样的回答。
